...
class AcousticFeatures:
    ...
    emphasis_segments: list = field(default_factory=list)  # [{word, start, end, energy_ratio}]
    tone_classification: str = "neutral"     # angry|frustrated|neutral|enthusiastic|sad|sarcastic
    ...
    def to_description(self) -> str:
        ...
        if self.emphasis_segments:
            top = sorted(self.emphasis_segments, key=lambda x: x.get("energy_ratio", 0), reverse=True)[:3]
        ...
        if self.pause_count > 3:
            parts.append(f"{self.pause_count} pauses (longest {self.longest_pause_seconds:.1f}s)")
...
class AudioProcessor:
    ...
    def _load_whisper(self):
        if self._whisper is None:
            ...
            self._whisper = whisper.load_model(self._whisper_model_name, device=self._device)
    ...
    def process(self, audio_path: str) -> AcousticFeatures:
        ...
        features.tone_classification, features.tone_confidence = self._classify_tone(features)
    ...
    def _extract_prosody(self, audio_path: str, word_segments: list[dict]) -> AcousticFeatures:
...
class MediaLoader:
    ...
    def load_from_row(self, row: dict) -> ProcessedMedia:
        ...
        if not media.audio_transcript and row.get("modality") in ("audio_transcript", "multimodal"):
    ...
    def validate_media(self, dataset_path: str) -> dict:
        ...
        stats = {
            "total": len(df),
            "needs_image": 0, "has_image": 0, "missing_image": 0,
            "needs_audio": 0, "has_audio": 0, "missing_audio": 0,
            "missing_images": [], "missing_audio_files": [],
        }
