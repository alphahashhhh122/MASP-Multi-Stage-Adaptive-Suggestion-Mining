...
def run_pipeline(text, images=None, audio_transcript=None,
                 sample_id=None, metadata=None, debug=False):
    ...
    state = {**_EMPTY_STATE,
        ...
        "raw_text": text, "raw_images": images or [],
    ...
    if state["domain"] not in VALID_DOMAINS:
        logger.warning(f"Domain '{state['domain']}' not in {VALID_DOMAINS} — defaulting to 'tech_software'")
...
def print_results(state: dict):
    ...
    print(f"  MODALITIES: text={'Y' if state.get('clean_text') else 'N'}  "
    ...
    if evidence_maps:
        ...
        for emap in evidence_maps:
            ...
            print(f"         pieces={emap.get('total_evidence_pieces', 0)}  modalities={mods}")
            print(f"         text={n_text}  image={n_img}  audio={n_audio}  cross_modal={n_cross}")
    ...
    if switch_stats:
        ...
        print(f"    Explicit: {switch_stats.get('explicit_count', 0)}  "
              f"Implicit: {switch_stats.get('implicit_count', 0)}")
        print(f"    Passed: {switch_stats.get('passed_count', 0)}  "
    ...
    if ranked:
        ...
        for r in ranked:
            ...
            print(f"       mode={r.get('view_weighting_mode')}  "
