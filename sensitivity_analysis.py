        ...
        preds.append(
            {
                ...
                "switch_mode": "COMMON"
                if cma.get("overall_alignment", 1.0) >= tau
                else "SPECIFIC",
            }
        )
...
def main():
    ...
    for tau in args.tau_values:
        ...
        results.append(r)
        logger.info(
            f"  tau={tau}: F1={r['F1']:.3f} COMMON={r['n_common']} SPECIFIC={r['n_specific']}"
        )
    ...
    for r in results:
        print(
            f"{r['tau']:>6.1f} {r['F1']:>8.3f} {r['P']:>8.3f} {r['R']:>8.3f} {r['n_common']:>8} {r['n_specific']:>10}"
        )
