...
import csv, json, sys, time, re, argparse, requests
...
def run_one(text, img="", aud=""):
    ...
    try:
        ...
        if match:
            ...
            if isinstance(v, str): v = v.lower() == "true"
...
def main():
    ...
    for i, r in enumerate(rows):
        ...
        pred, sug = run_one(r["raw_text"], r.get("image_description",""), r.get("audio_description",""))

        if pred and gold: tp+=1
        elif pred and not gold: fp+=1
        elif not pred and gold: fn+=1
        else: tn+=1
        results.append({"entry_id": r["entry_id"], "gold": gold, "pred": pred, "suggestion": sug})
    ...
    with open(f"{a.output}/b6_results.json", "w") as f:
        json.dump({"precision":pr,"recall":rc,"f1":f1,"tp":tp,"fp":fp,"fn":fn,"tn":tn,"results":results}, f, indent=2)
