#!/usr/bin/env python3
"""
baselines_comprehensive.py — All baselines for EMNLP paper.
...
Dataset validation: B1-B3 prove dataset is a valid benchmark
  - If classical gets F1 0.3-0.6: dataset has signal but isn't trivial
  - MASP should beat all significantly
...
def baseline_tfidf_svm(train_df, test_df):
    ...
    pipe = Pipeline([
        ("tfidf", TfidfVectorizer(max_features=5000, ngram_range=(1, 2), sublinear_tf=True)),
        ("clf", LinearSVC(max_iter=5000, C=1.0, class_weight="balanced", random_state=42))
    ])
...
def evaluate(y_true, y_pred, name):
    ...
    mcc = mcc_num / mcc_den
    bal_acc = 0.5 * (tp/(tp+fn) if tp+fn else 0) + 0.5 * (tn/(tn+fp) if tn+fp else 0)
    return {"name": name, "P": round(p, 4), "R": round(r, 4), "F1": round(f1, 4), "Acc": round(acc, 4), "MCC": round(mcc, 4), "BalAcc": round(bal_acc, 4)}
...
def main():
    ...
    parser.add_argument("--baseline", default="all", choices=["all", "classical", "llm", "B1", "B2", "B3", "B4", "B5"])
    ...
    # Print table
    print(f"\n{'='*60}")
    print(f"BASELINE COMPARISON (Table 2)")
    print(f"{'='*60}")
    print(f"{'System':<25} {'P':>6} {'R':>6} {'F1':>6} {'MCC':>6} {'BalAcc':>6}")
    print("-" * 70)
    for name, m in results.items():
        print(f"{m['name']:<25} {m['P']:>6.3f} {m['R']:>6.3f} {m['F1']:>6.3f} {m['MCC']:>6.3f} {m['BalAcc']:>6.3f}")
    
    print(f"\nDataset validation: If B1-B3 get F1 in [0.3, 0.6], dataset has")
    print(f"signal but isn't trivially solvable by classical methods.")
