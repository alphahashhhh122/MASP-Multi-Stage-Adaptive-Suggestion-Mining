...
class EpisodicMemory:
    ...
    def add(self, text: str, decision: str, human_approved: bool = False):
        ...
        self._short_term = [
            e for e in self._short_term if datetime.fromisoformat(e["ts"]) >= cutoff
        ]
...
class SemanticMemory:
    ...
    def add(self, canonical_text: str, metadata: dict, persist: bool = True):
        ...
        if persist:
            if self._available:
                self._col.add(
                    ids=[doc_id], documents=[canonical_text], metadatas=[metadata]
                )
            else:
    ...
    def query(self, text: str, top_k: int = 3) -> list[dict]:
        ...
        for entry in self._short_term.values():
            ...
            if sim > 0.4:
                results.append(
                    {"canonical_text": entry["text"], "similarity": sim, **entry}
                )
        if self._available and self._col.count() > 0:
            try:
                r = self._col.query(
                    query_texts=[text], n_results=min(top_k, self._col.count())
                )
                for doc, meta, dist in zip(
                    r["documents"][0], r["metadatas"][0], r["distances"][0]
                ):
                    results.append(
                        {"canonical_text": doc, "similarity": 1.0 - dist, **meta}
                    )
            except Exception:
                pass
        else:
    ...
    def count(self) -> int:
        base = (
            self._col.count()
            if self._available
            else len(getattr(self, "_long_term_fallback", {}))
        )
...
class ModalityAlignMemory:
    ...
    def record(
        self, alignment_score: float, dominant_modality: str, domain: str, decision: str
    ):
        entry = {
    ...
    def expected_acceptance(
        self, alignment_score: float, dominant_modality: str, domain: str
    ) -> float:
        similar = [
            e
        ...
        if len(similar) < 3:
            return 0.5
        return round(
            sum(1 for e in similar if e["decision"] == "accept") / len(similar), 3
        )
        ...
        patterns = [
            {
                **v,
                ...
                "acceptance_rate": round(v["accepted_count"] / v["count"], 3)
                if v["count"]
                else 0,
            }
    ...
    def stats(self) -> dict:
        return {
            ...
            "cross_domain_count": sum(
                1 for p in self._patterns.values() if len(p["domains"]) >= 2
            ),
        }
...
class MemoryManager:
    ...
    def lookup(
        ...
        for s in suggestions:
            ...
            results[text] = {
                ...
                "semantic_memory_hit": memory_hit,
    ...
    def apply_memory_to_suggestions(
        self,
        suggestions,
        alignment_score=0.5,
        dominant_modality="text",
            ...
            enriched.append(
                {
                    **s,
                    ...
                    "confidence": mem.get(
                        "boosted_confidence", s.get("confidence", 0.5)
                    ),
