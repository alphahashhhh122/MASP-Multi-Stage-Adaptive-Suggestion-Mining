...
Usage:
    # Run full dataset
    python run_dataset.py --csv data/MASP_Dataset_v5_2500_FINAL.csv --output results/

    # Run with sample limit
    python run_dataset.py --csv data/test.csv --max-samples 50
...
from main import print_results, run_pipeline
...
def run_dataset(
    dataset_path, output_dir="results", split=None, max_samples=None, debug=False
):
    ...
    for i, (_, row) in enumerate(df.iterrows(), 1):
        ...
        if domain not in ("tech_software", "restaurant"):
            logger.warning(
                f"{eid}: domain='{domain}' invalid, defaulting to 'tech_software'"
            )
            domain = "tech_software"
        ...
        try:
            ...
            out = {
                ...
                if (cma.get("overall_alignment") or 1) < 0.6
                else "COMMON",
            ...
            if ranked:
                print(
                    f"  → {ranked[0]['text'][:60]} (score={ranked[0].get('score', '?')})"
                )
            else:
    ...
    print(
        f"  With image desc: {rdf.get('has_image_desc', pd.Series(dtype=bool)).sum()}"
    )
    print(
        f"  With audio desc: {rdf.get('has_audio_desc', pd.Series(dtype=bool)).sum()}"
    )
    print(f"  Errors: {rdf['error'].notna().sum()}")
    print(
        f"  Total time: {total_time / 60:.1f} min ({total_time / len(rdf):.1f}s/sample avg)"
    )
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="MASP Pipeline Runner (v5 description-first)"
    )
    p.add_argument("--text", help="Review text (single mode)")
    p.add_argument(
        "--domain", default="tech_software", choices=["tech_software", "restaurant"]
    )
    ...
    if a.text:
        r = run_single(
            a.text, a.domain, a.image_desc, a.audio_desc, a.sample_id, a.debug
        )
        print_results(r)
