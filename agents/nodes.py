...
def _detect_discourse_markers(text: str) -> bool:
    markers = ["also,", "also ", "but ", "but,", "additionally", "on the other hand",
...
def _compute_span_similarities(sentences: list[dict]) -> list[dict]:
    if len(sentences) <= 1:
        for s in sentences:
            s["starts_new_topic"] = False; s["shared_entities"] = []
        return sentences
    def keywords(text):
    ...
    for i, sent in enumerate(sentences):
        ...
        for j, other_kw in enumerate(kw_sets):
            if i != j: shared |= (kw_sets[i] & other_kw)
...
def preprocess_node(state: PipelineState) -> dict:
    ...
    updates, all_msgs = {}, []
    try:
        result, msgs = _call_llm(TEXT_PREPROCESS_SYSTEM, TEXT_PREPROCESS_USER.format(text=state["raw_text"]))
        enriched = _compute_span_similarities(result.get("sentences", []))
        updates.update({"clean_text": state["raw_text"].strip(), "sentences": enriched,
    ...
    for raw_bytes in state.get("raw_images", []):
        try:
            ...
            caption_result, msgs = _call_llm_vision(IMAGE_CAPTION_SYSTEM, caption_user, b64)
            image_captions.append(caption_result); all_msgs.extend(msgs)
        except Exception as e:
            logger.warning(f"Image caption failed: {e}")
            image_captions.append({"caption": "Image could not be processed", "ui_state": "normal",
                                   "visible_problem": None, "missing_elements": [], "ui_elements": [],
                                   "emotional_context": "neutral", "suggestion_implied": None})
    updates.update({"image_captions": image_captions, "image_base64": image_b64_list,
    ...
    if not audio_transcript and state.get("raw_audio"):
        try:
            ...
            logger.info(f"  Audio transcribed: {len(audio_transcript)} chars, tone={features.tone_classification}")
        except Exception as e:
...
def text_view_builder_node(state: PipelineState) -> dict:
    ...
    user = TEXT_VIEW_BUILDER_USER.format(text=state["clean_text"],
        sentences=json.dumps(state.get("sentences", []), indent=2),
        sentiment=state["sentiment"], angry=state["has_angry_markers"])
    try:
        result, msgs = _call_llm(TEXT_VIEW_BUILDER_SYSTEM, user)
    except Exception as e:
        return {"error": f"text_view_builder: {e}", "messages": []}
    return {"text_semantic_view": result.get("semantic"), "text_syntactic_view": result.get("syntactic"),
            "text_pragmatic_view": result.get("pragmatic"),
            "text_view_confidence": result.get("text_view_confidence", 0.5), "messages": msgs}
...
def image_view_builder_node(state: PipelineState) -> dict:
    ...
    if not state.get("has_images"):
        return {"image_semantic_view": None, "image_syntactic_view": None,
                "image_pragmatic_view": None, "image_view_confidence": 0.0, "messages": []}
    all_msgs, best_result, best_confidence = [], None, 0.0
    for i, (b64, cap) in enumerate(zip(state.get("image_base64", []), state.get("image_captions", []))):
        cap_str = cap.get("caption", "") if isinstance(cap, dict) else str(cap)
        try:
            user = IMAGE_VIEW_BUILDER_USER.format(text_context=state["clean_text"][:300], caption=cap_str)
            result, msgs = _call_llm_vision(IMAGE_VIEW_BUILDER_SYSTEM, user, b64)
            all_msgs.extend(msgs)
            conf = result.get("image_view_confidence", 0.0)
            if conf > best_confidence: best_confidence, best_result = conf, result
        except Exception as e:
            logger.warning(f"Image view build failed for image {i}: {e}")
    if best_result is None:
        return {"image_semantic_view": None, "image_syntactic_view": None,
                "image_pragmatic_view": None, "image_view_confidence": 0.0, "messages": all_msgs}
    return {"image_semantic_view": best_result.get("semantic"), "image_syntactic_view": best_result.get("syntactic"),
            "image_pragmatic_view": best_result.get("pragmatic"),
            "image_view_confidence": best_confidence, "messages": all_msgs}
...
def audio_view_builder_node(state: PipelineState) -> dict:
    ...
    if not state.get("has_audio") or not state.get("audio_transcript"):
        return {"audio_semantic_view": None, "audio_pragmatic_view": None,
                "audio_view_confidence": 0.0, "messages": []}
    ...
    user = AUDIO_VIEW_BUILDER_USER.format(
        ...
        acoustic_features=acoustic_str)
    try:
        result, msgs = _call_llm(AUDIO_VIEW_BUILDER_SYSTEM, user, fast=True)
    except Exception as e:
        return {"error": f"audio_view_builder: {e}", "messages": []}
    return {"audio_semantic_view": result.get("semantic"), "audio_pragmatic_view": result.get("pragmatic"),
            "audio_view_confidence": result.get("audio_view_confidence", 0.0), "messages": msgs}
...
def cross_modal_align_node(state: PipelineState) -> dict:
    ...
    cap_str = first_cap.get("caption", "No image") if isinstance(first_cap, dict) else str(first_cap)
    user = CROSS_MODAL_ALIGNMENT_USER.format(
        raw_text=state["clean_text"], true_intent=text_sem.get("true_intent", "unknown"),
        ...
        image_shows_error=(state.get("image_pragmatic_view") or {}).get("shows_error", False),
...
def domain_router_node(state: PipelineState) -> dict:
    ...
    return {"domain": state.get("domain", "tech_software"), "domain_jargon": [], "messages": []}
...
def conservative_labeller_node(state: PipelineState) -> dict:
    ...
    captions = state.get("image_captions") or []
    first_cap = captions[0] if captions else {}
    user = CONSERVATIVE_LABELLER_USER.format(domain=state.get("domain", "general"),
    ...
    for s in suggestions: s["labeller"] = "conservative"
...
def liberal_labeller_node(state: PipelineState) -> dict:
    ...
    cma = state.get("cross_modal_alignment") or {}
    captions = state.get("image_captions") or []
    user = LIBERAL_LABELLER_USER.format(domain=state.get("domain", "general"),
        text=state["clean_text"],
        text_semantic=json.dumps(state.get("text_semantic_view"), indent=2),
        text_syntactic=json.dumps(state.get("text_syntactic_view"), indent=2),
    ...
    suggestions = result.get("suggestions", [])
    for s in suggestions: s["labeller"] = "liberal"
...
def arbitration_node(state: PipelineState) -> dict:
    ...
    text_prag = state.get("text_pragmatic_view") or {}
    cma = state.get("cross_modal_alignment") or {}
    user = ARBITRATION_USER.format(conservative_labels=json.dumps(conservative, indent=2),
        liberal_labels=json.dumps(liberal, indent=2), domain=state.get("domain", "general"),
        overall_alignment=cma.get("overall_alignment", 0.5),
        dominant_modality=cma.get("dominant_modality", "text"),
        urgency_score=text_prag.get("urgency_score", 0.5))
    try:
    ...
    return {"accepted_suggestions": result.get("accepted", []),
            "rejected_suggestions": result.get("rejected", []), "messages": msgs}
...
def canonicaliser_node(state: PipelineState) -> dict:
    ...
    ENRICH_FIELDS = [
        "evidence_grounding_score", "total_evidence_pieces", "evidence_modalities",
        "suggestion_type", "faithfulness_score", "actionability_score",
        "feasibility_score", "specificity_score", "inference_chain",
        "type_switch_passed", "span_verified", "action_verb", "action_target",
    ]
...
def memory_node(state: PipelineState) -> dict:
    ...
    enriched = mem.apply_memory_to_suggestions(
        ...
        dominant_modality=cma.get("dominant_modality", "text"),
        domain=state.get("domain", "general"))
    return {"canonical_suggestions": enriched,
            "memory_hits": mem.lookup(state.get("canonical_suggestions", []),
...
def reranker_node(state: PipelineState) -> dict:
    ...
    mem = get_memory_manager()
    for item in ranked:
        mem.record_decision(item["text"], "accept", human=False,
            alignment_score=cma.get("overall_alignment", 0.5),
            dominant_modality=cma.get("dominant_modality", "text"),
            domain=state.get("domain", "general"))
...
def human_review_gate_node(state: PipelineState) -> dict:
    ...
    for item in state.get("ranked_suggestions", []):
        ...
        if item.get("score", 10) < 6.0: reasons.append("low_score")
        if not feats.get("memory_hit", True): reasons.append("novel_suggestion")
        if feats.get("dominant_modality") == "image" and not feats.get("text_corroborated"):
            reasons.append("image_only_no_text_corroboration")
        if has_contradiction: reasons.append("cross_modal_contradiction")
        grounding = feats.get("grounding_score", feats.get("evidence_grounding_score", 1.0))
        if grounding < 0.40: reasons.append("low_grounding_faithfulness")
        if feats.get("suggestion_type") == "implicit" and feats.get("actionability_score", 5) < 3.0:
            reasons.append("implicit_low_actionability")

        if reasons:
            uncertain.append({"text": item["text"], "score": item.get("score"),
                              "reasons": reasons, "dominant_modality": item.get("dominant_modality"),
