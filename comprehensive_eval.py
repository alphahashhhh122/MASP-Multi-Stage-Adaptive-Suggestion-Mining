...
def binary_metrics(y_true, y_pred):
    ...
    bal_acc = (tpr+tnr)/2
    return {"P":round(p,4),"R":round(r,4),"F1":round(f1,4),"Acc":round(acc,4),
            "MCC":round(mcc,4),"BalAcc":round(bal_acc,4),
...
def bootstrap_ci(y_true, y_pred, metric="F1", n_boot=1000, alpha=0.05):
    ...
    return round(np.percentile(scores, 100*alpha/2), 4), round(np.percentile(scores, 100*(1-alpha/2)), 4)
...
def implicit_gate_quality(results_df, test_df):
    ...
    detected = implicit[implicit.get("num_suggestions", pd.Series(dtype=float)).fillna(0) > 0]
    
    return {
        "detection_rate": round(len(detected)/len(implicit), 4) if len(implicit) else 0,
        "gate_pass_rate": round(len(detected)/len(implicit), 4) if len(implicit) else 0,
        "avg_score": round(detected["top_score"].mean(), 4) if len(detected) > 0 and "top_score" in detected.columns else 0,
        "n": len(implicit)
    }
...
def ranking_metrics(results_df, test_df):
    ...
    merged = test_df.merge(results_df, on="entry_id", how="left", suffixes=("","_p"))
    has_sugg = merged[(merged["is_suggestion"]==1) & (merged.get("num_suggestions", pd.Series(dtype=float)).fillna(0) > 0)]
...
def switch_metrics(results_df, test_df):
    ...
    return {
        ...
        "n": total,
        "specific_correct": int(sum(1 for _, r in merged.iterrows() if r.get("extraction_path","") in specific_paths and str(r.get(sw_col,""))=="SPECIFIC")),
        "common_correct": int(sum(1 for _, r in merged.iterrows() if r.get("extraction_path","") in common_paths and str(r.get(sw_col,""))=="COMMON")),
    }
...
def run_full_evaluation(results_csv, test_csv, output_dir):
    ...
    for path in sorted(merged["extraction_path"].unique()):
        ...
        m = binary_metrics(yt, yp)
        per_path[path] = m
        print(f"    {path:5s}: P={m['P']:.2f} R={m['R']:.2f} F1={m['F1']:.2f} MCC={m['MCC']:.2f} (n={len(pm)})")
    ...
    print(f"  Explicit (n={explicit['n']}): span_exact={explicit['span_exact_match']:.3f} span_partial={explicit['span_partial_match']:.3f}")
    print(f"  Implicit (n={implicit['n']}): detection_rate={implicit['detection_rate']:.3f} avg_score={implicit.get('avg_score',0):.3f}")
    ...
    for stype in ["explicit", "implicit"]:
        ...
        if len(st) > 0:
            ...
            print(f"  {stype:10s}: P={m['P']:.2f} R={m['R']:.2f} F1={m['F1']:.2f} (n={len(st)})")
    ...
    print(f"  SPECIFIC correct={switch.get('specific_correct',0)} COMMON correct={switch.get('common_correct',0)}")
    ...
    print("\n▸ DATA INTEGRITY:")
    seed = merged[~merged["entry_id"].str.startswith("GEN") & ~merged["entry_id"].str.startswith("HN_")]
    gen = merged[merged["entry_id"].str.startswith("GEN") | merged["entry_id"].str.startswith("HN_")]
    for name, df in [("Seed", seed), ("Generated", gen)]:
        if len(df) > 0:
            ...
            print(f"  {name:12s}: F1={m['F1']:.3f} [{ci[0]:.3f},{ci[1]:.3f}] MCC={m['MCC']:.3f} (n={len(df)})")
