...
def evaluate(y_true, y_pred):
    ...
    return {
        ...
        "MCC": round(mcc, 4), "BalAcc": round(bal, 4),
        "TP": tp, "FP": fp, "FN": fn, "TN": tn,
        "n": len(y_true), "pos": sum(y_true),
...
def main():
    ...
    log.info(f"SemEval train: {len(semeval_train)} samples "
             f"({semeval_train['label'].sum()} pos, "
             f"{len(semeval_train) - semeval_train['label'].sum()} neg)")
    log.info(f"SemEval test:  {len(semeval_test)} samples "
             f"({semeval_test['label'].sum()} pos, "
             f"{len(semeval_test) - semeval_test['label'].sum()} neg)")

    # ── Load MASP ──
    masp_train, masp_test = load_masp()
    log.info(f"MASP train: {len(masp_train)} samples "
             f"({masp_train['is_suggestion'].sum()} pos)")
    log.info(f"MASP test:  {len(masp_test)} samples "
             f"({masp_test['is_suggestion'].sum()} pos)")
    ...
    for dataset_name, train_df, test_df, text_col, label_col in [
        ...
        for bname, fn in [
            ("B1: Rule-based",
             lambda: baseline_b1_rules(texts)),
            ("B2: TF-IDF+LogReg",
             lambda: baseline_b2_tfidf(train_texts, train_labels, texts)),
            ("B7: Keyword+Sent",
             lambda: baseline_b7_keyword_sentiment(texts)),
        ]:
            preds = fn()
            r = evaluate(labels, preds)
            results[(dataset_name, bname)] = r
            print(f"  {bname:<22} P={r['P']:.3f} R={r['R']:.3f} "
    ...
    print(f"{'=' * 74}")
    print(f"{'Method':<22} | {'SemEval F1':>10} {'MCC':>6} | "
    ...
    for bname in ["B1: Rule-based", "B2: TF-IDF+LogReg", "B7: Keyword+Sent"]:
        ...
        print(f"{bname:<22} | {se.get('F1', 0):>10.3f} {se.get('MCC', 0):>6.3f} | "
              f"{ma.get('F1', 0):>8.3f} {ma.get('MCC', 0):>6.3f} | {diff:>+7.3f}")

    # Published reference
    print(f"{'SemEval-2019 Winner':<22} | {'0.858':>10} {'---':>6} | "
          f"{'---':>8} {'---':>6} |")

    print(f"\nInterpretation:")
    print(f"  If |F1 diff| < 0.15 → datasets have comparable difficulty")
    print(f"  If MASP B1 F1 in [0.3, 0.6] → dataset valid, not trivially solvable")
