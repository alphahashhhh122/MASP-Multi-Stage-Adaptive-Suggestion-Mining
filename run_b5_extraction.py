...
def main():
    ...
    ext_values = [r["extraction_overlap"] for r in rows if r["gold"] == 1 and r["pred"] == 1]
    ext = sum(ext_values) / len(ext_values) if ext_values else 0.0
    mining_f1 = 2 * det_f1 * ext / (det_f1 + ext) if det_f1 + ext else 0.0
    print(f"P={precision:.3f} R={recall:.3f} DetectionF1={det_f1:.3f} Ext={ext:.3f} MiningF1={mining_f1:.3f}")
