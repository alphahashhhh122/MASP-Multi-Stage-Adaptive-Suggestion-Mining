...
def stem(word):
    word = word.lower()
    for suffix in ['ing','tion','ment','ness','able','ible','ful','less','ous','ive','ly','ed','er','es','s']:
        if word.endswith(suffix) and len(word) > len(suffix) + 2:
            return word[:-len(suffix)]
    return word

def stemmed_overlap(pred_text, gold_text):
    pred_stems = set(stem(w) for w in re.findall(r'\w+', pred_text.lower()) if len(w) > 2)
    gold_stems = set(stem(w) for w in re.findall(r'\w+', gold_text.lower()) if len(w) > 2)
    if not pred_stems or not gold_stems: return 0
...
def compute_metrics(subset):
    ...
    mcc = mcc_num/mcc_den
    return {'tp':tp,'fp':fp,'fn':fn,'tn':tn,'P':P,'R':R,'F1':F1,'MCC':mcc,'n':len(subset)}
...
for path in ['P1','P2','P3','P4','P5','P6','P7','P8','HN']:
    ...
    if s:
        ...
        print(f"  {path}: n={m2['n']:>3} TP={m2['tp']:>3} FP={m2['fp']:>3} FN={m2['fn']:>3} R={m2['R']:.3f} F1={m2['F1']:.3f}")
...
for c in ['T','T+I','T+A','T+I+A']:
    ...
    if s:
        m2 = compute_metrics(s)
        print(f"  {c}: F1={m2['F1']:.3f} P={m2['P']:.3f} R={m2['R']:.3f} MCC={m2['MCC']:.3f}")
...
overlaps_p578 = []
for p in preds:
    if p['pred']==1 and p['gold']==1 and p['top_suggestion'] and p['gold_suggestion']:
...
overlaps.sort()
print(f"\nEXTRACTION QUALITY:")
print(f"  All TP: mean={sum(overlaps)/len(overlaps):.3f} median={overlaps[len(overlaps)//2]:.3f} n={len(overlaps)}")
if overlaps_p578:
    print(f"  P5/P7/P8: mean={sum(overlaps_p578)/len(overlaps_p578):.3f} n={len(overlaps_p578)}")

# MINING F1
det_f1 = compute_metrics(preds)['F1']
ext_quality = sum(overlaps)/len(overlaps)
mining_f1 = 2 * det_f1 * ext_quality / (det_f1 + ext_quality) if det_f1+ext_quality else 0
...
KEYWORDS = ['should','could','need','wish','would','please','hope','suggest','add','fix','improve','better','more']
baseline_overlaps = []
for r in test_data:
    if r['is_suggestion'] != 'True': continue
    text = r['raw_text']
    gold_sug = r.get('suggestion_text', '')
    if not gold_sug: continue
...
baseline_ext = sum(baseline_overlaps)/len(baseline_overlaps) if baseline_overlaps else 0
...
print("-"*55)
print(f"{'B3 TF-IDF+SVM':<25} {svm_F1:>8.3f} {baseline_ext:>8.3f} {2*svm_F1*baseline_ext/(svm_F1+baseline_ext):>10.3f}")
